  ...
  define: {
    extend: {
      set (set, val, stamp, nocontext, isNew) {
        ...
        if ((typeof val !== 'object' || !val) && stamp && this.stamp) {
          const p = vstamp.parse(stamp)
          // if (p.src) {
          if (Number(p.val) < Number(vstamp.parse(this.stamp).val)) {
            console.log('OLD WRONG', p, this.path(), val, this.stamp)
            console.log(p.val, '>', vstamp.parse(this.stamp).val, p.val > vstamp.parse(this.stamp).val)

'use strict'
const subscribe = require('vigour-state/subscribe')
const serialize = require('./serialize')
const parse = require('./parse')
const vstamp = require('vigour-stamp')
const dummy = () => {}
const hash = require('quick-hash')

exports.define = {
  subscribe (subs, update, tree, stamp, client, id) {
    const hub = this.root
    if (!update) { update = dummy }
    if (client && client.isClient) {
      subs = parse(subs, hub)

      const p = (tree) => {
        var arr = []
        var key = tree._key
        while (tree && tree._key) {
          key = tree._key
          if (key === '$root') {
            arr = []
          } else if (key === '$parent') {
            console.log('handle parent! in temp solution')
          } else if (key[0] !== '$') {
            arr.unshift(tree._key)
          }
          tree = tree._p
        }
        return hub.get(arr)
      }

      update = (state, type, stamp, subs, tree, sType) => {
        // console.log('update', client.id, state && state.path())

        // also need to take care of this case in normal subscribe -- need to handle it for brisky
        // 1 has to fire for ref changes i geuss...
        var r
        if (!state) {
          // need parent in subs for hub
          // need to see if its allrdy there
          if (tree._key === '$current') {
            console.log(' ok reason')
            r = true
            type = 'update'
            subs = { val: 1 }
            state = p(tree._p._p)
          }
        }

        if (
          state && state.syncDown && (!state.syncDownIsFn || state.syncDown(state))
        ) {
          if (type !== 'remove' || state.val === null) {
            const stamp = state.stamp
            if (r) console.log('xxx', stamp)
            if (stamp) {
              const src = vstamp.src(stamp)
              if (client.key !== src) {
                if (r) console.log('go send!', state)
                client.send(state, type, stamp, subs, tree, sType)
              }
            }
          }
        }
      }
    }

    // add re-use subscription (use same tree for same subs will handle this in state)
    tree = subscribe(this, subs, update, tree, stamp, client, id)

    if (!client || !client.isClient) {
      let hubSubs = hub.subscriptions
      if (!hubSubs) { hubSubs = hub.subscriptions = {} }
      const path = this.realPath(false, true)
      if (path.length > 0) {
        console.log('subs -- need to resolve path -- do a bit later -- deeper subs are currently not supported')
        // FIX THIS
      } else {
        const serialized = serialize(this, subs)
        if (!id) { id = hash(serialized) }
        hubSubs[id] = serialized
      }
      if (hub.client) { hub.client.origin().sendMeta() }
    }
    // tree re-use over multiple clients if they have the same subs
    // totally possible just hash the incoming
    return tree
  }
}

  ...
  subscriptionWalker: function subscriptionWalker (key, hub, map, event, result, client, top, path) {
    ...
    for (let key in map) {
      if (key === '*') {
        ...
        console.log('go parent!', parentResult)
        let p = obs && obs.parent
        let spath = p ? p.syncPath : path.slice(0, -1)
        let presult = top
        for (let segment in spath) {
          presult = presult[spath[segment]]
        }
        console.log('-------->', spath)
        subscriptionWalker.call(p, void 0, hub, map[key], event, presult, client, top, spath)
      } else {

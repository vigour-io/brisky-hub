...
module.exports = function single (key, hub, map, event, result, client, top) {
  var path = this.syncPath
  hub.subscribe(path.length > 1 ? path : true, 'data', [
    function (data, event, client) {
      if (client.scope === hub.scope) {
        // do check here faster.. for if myself
  ...
  console.log('yoyoyoy!', this.path, this._iput)
  // handle ref
  // wrong!

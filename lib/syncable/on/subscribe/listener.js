...
module.exports = function (load, event, client) {
  console.log('\n\n\nFIRE SUBSCRIPTION'.cyan.bold.inverse, ' --> client:', client.key.blue.bold, client.path.join('.'))
  if (client.parent.parent._scope) {
    console.log('SCOPE'.magenta, client.parent.parent._scope)
  }
  console.log('wtf wtf wtf'.green.bold, client.serialize(), this.path.join('.'))
  // if(client.parent.parent)
  var adapter = this.getAdapter()
  if (!adapter) {
    return
  }
  // if(client.scope !== this.scope) {
  //   continue
  // }

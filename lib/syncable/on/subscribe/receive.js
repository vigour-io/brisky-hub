  ...
  receiveSubscription (subscriptions, id, event) {
    ...
    for (var i in subscriptions) {
      ...
      for (var j in subscriptions[i]) {
        if (!j || !i) {
          console.log('fraudulent request old crap', j)
          return
        }
        let path = j.split('.')
        if (!path || path[0] === void 0) {
          console.log('fraudulent request old crap', j)
          return
        }
        // console.log
        if (subscriptions[i][j].property || subscriptions[i][j].data) {
          console.log('probably old dont do!')
          return
        }
        console.log('hello!', 'dogin it', i, j, path, subscriptions[i][j], response, path)
        ...
        handleSubs(
          ...
          void 0,
          client,
          path[0] === '$' ? response : setWithPath(response, path, {}),
          ev,
          response,
          hub._scope,
          hub

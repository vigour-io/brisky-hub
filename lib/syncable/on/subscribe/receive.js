  ...
  receiveSubscription (subscriptions, id, event) {
    ...
    for (var i in subscriptions) {
      if (typeof i !== 'string' || i.length > 7 || i.indexOf('.') > -1) {
        console.log('FUCK WRONG SUBSCRIPTION-HASH LETS TRY TO MITIGIATE THE DMG BY RETURNING --> bad:', i)
        return
      }
    ...
    try {
      ...
      console.log('dirty error /w sending subs-payload', ev.stamp)
    }
    ev.trigger()
  }
}
...
function handleSubs (obs, map, key, client, returnobj, event, top, scope, hub) {
  if (!obs) {
    console.log('no obs in subscribe hub receive -- abort (?)', map)
    return
  }
  if (key) {

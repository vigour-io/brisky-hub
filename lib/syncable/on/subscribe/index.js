...
exports.define = {
  subscribe (pattern, fn, key, unique, event, nocnt) {
    ...
    console.log('bitch plz!'.blue, pattern)

    this.sendSubscriptionUpstream(subs.pattern, event, subs.key, nocnt)
    console.log('rdy bithz'.blue, pattern, key, subs.pattern)
    return subs
  },
    ...
    if (typeof key === 'string') {
      ...
      if (this.removeSubscription(key)) {
        ...
        // if (emitter) {
          ...
          // emitter.remove() //event clear or close
        // }
  ...
  receiveSubscription (subs, id, event) {
    // console.log('receiveSubscription:'.yellow, JSON.stringify(subs, false, 2))
    if (subs.unsubscribe) {
      // console.log('unsubscribe:'.red, subs.unsubscribe)
      let target = this
  ...
  receiveSubscriptionEach (subscribe, client, hashed, event) {
    if (subscribe) {
      ...
      // console.log('subscribe:'.blue, subscribe.val)
      let sub = target.subscribe(subscribe.val, [listener, client], client.key, true, event, true)
      if (!sub.onRemoveProperty) {
  ...
  onSub (event, emitter, pattern) {
    ...
    // console.log('sendSubscriptionUpstream'.blue, JSON.stringify(pattern.serialize(), false, 2))
  ...
  sendSubscriptionUpstream (pattern, event, key, nocnt) {
    ...
    console.log('dirty face', pattern, serialized)
    if (this.addSubscription(key, nocnt)) {
      console.log('ok send it up', key)

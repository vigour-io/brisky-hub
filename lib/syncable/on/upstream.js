  ...
  sendUpstream (data, event) {
    ...
    if (!event.upstream) {
      adapter.each(
        ...
        (property) => (property instanceof Protocol) && property.__input && property.client
      )
    }
  }
}
...
//       client.connection &&
//       client !== event.client &&
//       (client.connection && client.connection.origin.upstream && !client.connection.origin.upstream.__input) // up or down
//     ) {

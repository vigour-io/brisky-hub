...
module.exports = function (syncable, event) {
  ...
  exports.define = {
    ...
    onSub (data, event, emitter, pattern, current, mapvalue, map, context) {
      ...
      if (this.substore) {
        // if hash already in store don't do stuff
      ...
      // store the pattern hash
      this.substore[hashed] = true
      console.log('proceeding', JSON.stringify(pattern.serialize(function (property, key) {
        if (key !== 'key') {
          return true
        }
      }), false, 2))
      ...
      // add a listener to the holder of this reference (context)
      // maybe merge this with existing reference listener?
      // context.on('reference', [ function (data) {
      //   console.log('- reference changed: remove my stuff'.red.bold, data)
      // }, emitter])

      // send the subscription if I dont have this data already here
    },
    getAdapter () {
      let adapter
      let parent = this

...
exports.define = {
  subscribe (pattern) {
    ...
    console.log('lulllz subscribing!'.rainbow, subs.key)
    // if (!dontadd) {
    this.addSubscription(
      sendSubsUp(this, subs.pattern, void 0, subs.key)
    )
    // }
    return subs
  },
  ...
  addSubscription (key) {
    ...
    return this.subscollectors[key]
  },
  ...
  onSub (data, event, emitter, pattern, current, mapvalue, map, context) {
    ...
    let key = sendSubsUp(this, pattern, event)
...
function sendSubsUp (base, pattern, event, key) {
  if (!event) {

...
exports.on = {
  data: {
    adapter (data, event) {
      ...
      if (!adapter) {
        parent.emit('error', new Error('no adapter yet'))
        return
      }
      ...
      // console.log('?', eventorigin, event.stamp)

      if (adapaterClient && !event.upstream) {
        // if(this)
        // check if the event did not come from upstream
        // console.log('ok send it!', event.stamp)
      ...
      if (adapter.parent.clients) {
        adapter.parent.clients.each((client, key) => {
          if (key !== eventorigin && (!adapaterClient || client.origin !== adapaterClient)) {
            if(client.origin.lastStamp === event.stamp) {
              // console.log('OK WRONG!'.red, client.origin.lastStamp)
            } else {

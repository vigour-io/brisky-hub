      ...
      queue: function () {
        ...
        if (this.compute() === true && hub.upstream) {
          ...
          queue.client = {
            id: hub.id
            // context
            // subscriptions: {} // also need to add these guys -- store them in subscribe? -- can have multiple ofcourse
            // share multiple "connections" over one conneciton needs to be possible
            // so multiple clients as well
          }
          console.log('send')
          send(hub)
        }
...
function send (hub) {
  console.log('send up:', JSON.stringify(hub.queue, false, 2))

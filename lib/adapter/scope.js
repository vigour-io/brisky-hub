        ...
        this.parent.each(
          (property, key) => {
            ...
            if (property.client.origin.subscriptions) {
              console.log('yo subs!', property.client.origin.subscriptions)
  ...
  swtichScope (data, event) {
    if (data.previousScope) {
      ...
      if (client) {
        ...
        data.client.connection = client.connection.origin
        // connection is wrong!
        // console.log(client.listensOnAttach)
        // var ev = new Event()
        client.remove(event ) //has to be done now!
        // ev.trigger()
        // make sure to not comminciate
        console.log('SWITCH UP!, we need to clear some shit now')
      }
    }
    return data
  }

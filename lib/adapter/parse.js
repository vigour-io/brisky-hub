...
exports.define = {
  parse (data, connection) {
    ...
    if (!connection) {
      ...
      if (data.client) {
        ...
        if (uuid) {
          ...
          if (data.instance) {
            data.client.instance = instance = data.instance
          }
    ...
    if (data.set) {
      console.log('HEY INCOMING!', instance)
      if (instance) {
        if (hub.instanceMap[instance]) {
          console.warn('now doing a set on instance', data.instance, hub.instanceMap)
          hub.instanceMap[instance].set(data.set, event)
        } else {

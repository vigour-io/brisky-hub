      ...
      wsServer.on('request', (req) => {
        ...
        wsServerConn.on('message', (message) => {
          if (message.type === 'utf8') {
            this.emit('message', {data: message.utf8Data, connection: wsServerConn})
          } else if (message.type === 'binary') {
            this.emit('message', {data: message.binaryData, connection: wsServerConn})
          }
        })
        wsServerConn.on('close', () => {
          if (wsServerConn.client) {
            this.emit('close', wsServerConn.client.val)
          } else {
            this.emit('close')
          }
        })

  ...
  receive (data, connection) {
    ...
    if (data.subscribe) {
      ...
      if (connection !== 'upstream') {
        ...
        // console.log(client)
        // per client.id
        console.log('subscribins!', data.subscribe, client.key)

        var sub = (data.subscribe.path.length ? hub.get(data.subscribe.path, {}) : hub).subscribe(data.subscribe.val, [require('../../lib/syncable/on/subscription'), client], client.key)

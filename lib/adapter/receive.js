    ...
    if (connection === 'upstream') {
      ...
      if (data.client) {
        ...
        if (subs) {
          ...
          console.log('set subs seem to work', subs)
        ...
        if (connection.ip) {
          ...
          if (scope) {
            // get all clients by ip as well....
            // make a map for now?
            // add remove extension -- then ip
            // setTimeout(function () {
            //   if (hub.clients[id] && hub.clients[id]._input !== null) {
            //     var ev = new Event('data')
            //     var clientsback = {}
            //     hub.clients.each((p, key) => {
            //       if (p._input === null || key === id) {
            //         return
            //       }
            //       clientsback[key] = p.serialize && p.serialize()
            //     })
            //     ev.trigger()
            //     hub.clients[id].connection.origin.push({
            //       set: {
            //         clients: clientsback
            //       },
            //       stamp: ev.stamp

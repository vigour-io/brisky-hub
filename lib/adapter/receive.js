          ...
          if (subs.unsubscribe) {
            ...
            if (target) {
              for (let hashed in subs.unsubscribe.val) {
                ...
                console.log('REMOVE'.red, hashed, id)
...
function dosubs (hub, subscribe, client, hashed) {
  if (subscribe) {
    ...
    if (target._on && (!target._on[hashed] || !target._on[hashed].attach || !target._on[hashed].attach[client])) {
      var sub = target.subscribe(subscribe.val, [require('../../lib/syncable/on/subscription'), client], client.key)
      // console.log('HERE'.rainbow, sub)

      // use last stamp
      console.log('START HERE'.inverse.red)
      console.log('!!!skered!!!', subscribe.val)
      sub.run()
      console.log('END HERE'.inverse.red)
    }

...
exports.define = {
  receive (data, connection) {
    ...
    console.log('in'.magenta, JSON.stringify(data, false, 2), this._path.join('.'))
    if (
      typeof stamp === 'string' &&
    ...
    if (data.subscribe) {
      ...
      if(connection !== 'upstream') {
        ...
        // per client.id
        var sub = hub.subscribe(data.subscribe, [require('../../lib/syncable/on/subscription'), client ], client.key)

...
function dosubs (hub, subscribe, client, hashed) {
  if (subscribe) {
    ...
    if (target._on && (!target._on[hashed] || !target._on[hashed].attach || !target._on[hashed].attach[client])) {
      ...
      // do event here smart!
      //
      // (pattern, fn, key, unique, event, nocnt)
      var sub = target.subscribe(subscribe.val, [require('../../lib/syncable/on/subscribe/listener'), client], client.key, void 0, void 0, true)
      sub.onRemoveProperty = function () {

        // if (hub.clients[hub.adapter.id].subscriptions) {
        //
        // }
        // if (hub.clients[id].subscriptions) {
        //   delete hub.clients[id].subscriptions[subs.unsubscribe.val[j]]
        // }
        // if (!this.base && !this.fn && !this.attach) {

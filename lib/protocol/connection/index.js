'use strict'
var Observable = require('vigour-js/lib/observable')
// var _set = require('vigour-js/lib/observable/on/constructor').prototype.set
var Client = require('../../client')
// need to store have to know if one exists allready, hash characterictics or something
// e.g patterns or whatever do it fast
// what is the common characteristic?
// up + down + protocol type
// now get an iphone build asap and make kick ping etc
module.exports = exports = new Observable({
  properties: {
    reconnectTime: { val: 100 },
    reconnectMax: { val: 5000 },
    reconnectFactor: { val: 1.1 },
    reconnecting: true
  },
  on: {
    remove: {
      clients (data, event) {
        this._on.data.base.each(function (property, key) {
          if (property.parent instanceof Client) {
            property.parent.remove(event)
          }
        })
      }
    },
    connect: {
      reconnect () {
        if (this.reconnecting) {
          console.log('hello im cancelin!!'.rainbow)
          clearTimeout(this.reconnecting)
          this.reconnecting = null
        }
      }
    },
    reconnect: {
      reconnect (data, event) {
        console.log('hello im firin!'.rainbow)
        this.reconnecting = setTimeout(() => {
          this.upstream.emit('data', this.upstream.val)
          this.emit('reconnect', Math.min(data * this.reconnectFactor, this.reconnectMax))
        }, data)
      }
    },
    close: {
      reconnect (data, event) {
        this.emit('reconnect', this.reconnectTime, event)
      }
    }
  }
}).Constructor

// exports.prototype._on.define({
//   set (val, event) {
//     // importnt for what?
//     // console.log('something ON-like happening with on here???', val)
//     return _set.apply(this, arguments)
//   }
// })

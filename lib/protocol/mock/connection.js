...
module.exports = new Connection({
    ...
    on: {
      data: {
        protocol (data, event) {
          ...
          if (this._parent.internal) {
            this._parent.internal.emit('close')
            this._parent.internal._serverside.emit('close')
            this._parent.internal.remove(event)
          }

          var server = serverList[this.val]
          if (server) {
            var serverSide
            var internal = this._parent.internal = new Internal({
              on: {
                send (data) {
            ...
            internal.on('message', (data) => {
              this._parent.emit('message', data)
            })
            internal.on('connect', () => {
              this._parent.emit('connect')
            })
  ...
  define: {
    send (data) {
      this.mocktimer = setTimeout(() => this.internal.emit('send', JSON.parse(JSON.stringify(data))), 0)
    }
  },
  on: {
    remove: {

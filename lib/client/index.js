    ...
    send (observable, hub, data, event) {
      if (hub.scope || this.scope) {
        ...
        if (this.scope && hub.scope === this.scope) {
          ...
          if (hub.scope && hub.hasOwnProperty('adapter')) {
            console.log('llulz')
          } else {
            console.log('nope')
            // console.log('  update on instance and this client does NOT care!', hub.scope, hub.serialize(), hub.adapter.val)
            return
          }
        }
      } else {
      ...
      console.error('1-screre', observable._path, data)

    ...
    send (observable, hub, data, event) {
      if (this.connection && data !== void 0) {
        ...
        if (typeof data === 'object') {
          for (var i in data) {
            console.log(i) //dat is wrong
            //set on hub fires set on clients, for some reasons data is weird only client!
        ...
        currentStatus.lastSet = JSON.stringify(ret)+ ' '+stamp+ ' '+path.join('.')

    ...
    send (observable, hub, data, event) {
      if (this.connection && data !== void 0) {
        ...
        if (typeof stamp !== 'string') {
          ...
          // if(event.val && event.origin === observable) {
          //   // console.log('ok only now do it!')
          // } else {
          //   // console.log('no!'.red, event.val, event.origin.path)
          //   // return
          // }
        }
        ...
        // console.log(ret instanceof Buffer)
        if (ret instanceof Buffer) {
          var StringDecoder = require('string_decoder').StringDecoder
          var decoder = new StringDecoder('utf8')
          ret = decoder.write(ret)
        }
        // console.log(ret)
        currentStatus.out = (cnt++)

...
module.exports = new ClientSyncable({
  ...
  define: {
    ...
    send (observable, hub, data, event, subscriptions) {
      ...
      let type = typeof stamp
      if (
        this.connection &&
        (this._input && data !== void 0) &&
        (type !== 'string' || stamp.indexOf(this._input) !== 0)
      ) {
        if (type !== 'string' || !isNetworkStamp(stamp)) { // network stamp is slow
          stamp = id + seperator + stamp // this can just be done on the server
        }
        if (subscriptions) {
          // wtf is going on how to do
          console.log('yo?', stamp)

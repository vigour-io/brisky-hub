...
exports.define = {
  ...
  send (state, type, stamp, subs, tree, sType) {
    if (this.root.val !== null) {
      if (!subs || subs.val) {
      // state.val && state.val.isBase)
      // if (!subs || subs.val === true || (state.val === null && subs.val)) {
        // the context feels very arbitrary,, why would you do that? why not just false???

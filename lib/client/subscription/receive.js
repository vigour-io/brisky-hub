...
exports.define = {
  receiveSubscriptions (observable, hub, map, event, attach, hash) {
    ...
    console.log('⬇️    subscription incoming!', hash) //JSON.stringify(map, false, 2)
    ...
    if (map === null) {
      console.log('remove subs find in --> websocket.client.origin.subscriptions.' + path.join('.') + '.' + hash)

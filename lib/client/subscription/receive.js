...
exports.define = {
  receiveSubscriptions (observable, hub, map, event, attach, hash) {
    ...
    if (map === null) {
      ...
      console.log()
      console.log('remove subs find in --> websocket.client.origin.subscriptions.' + lPath + '.' + hash)
      map = hub.adapter.get([ 'websocket', 'client', 'origin', 'subscriptions', lPath, hash, '$map' ])
      if (!map) {
